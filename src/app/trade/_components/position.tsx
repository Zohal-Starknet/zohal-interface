    ...
    <table className="mt-6 w-full">
      <thead className="border-b border-neutral-800 text-left">
        <tr className="text-[#bcbcbd] flex space-x-2.5">
          ...
          <th className={clsx(tableHeaderCommonStyles, "hidden xl:block w-32")}>Collateral</th>
          <th className={clsx(tableHeaderCommonStyles, "hidden xl:block")}>Entry Price</th>
          <th className={clsx(tableHeaderCommonStyles, "hidden 2xl:block")}>Market Price</th>
          <th className={clsx(tableHeaderCommonStyles, "hidden 2xl:block")}>Liquidation Price</th>
        </tr>
      </thead>
      <tbody>
          ...
          return (
            <>
              <tr className="border-b border-neutral-800 text-sm flex items-center space-x-2.5" key={index}>
                ...
                <td className="hidden w-32 xl:block">
                ...
                <td className="hidden 2xl:block w-28">{data.markPrice}</td>
                <td className="hidden 2xl:block w-24">{data.liquidationPrice}</td>
                <td className="flex-1 flex justify-end">
                  <div className="flex items-center w-fit space-x-1">
                    ...
                    <button className="2xl:hidden" onClick={openModal}><EllipsisVerticalIcon className="h-8 w-6 text-white" /></button>
                  </div>
                </td>
              </tr>
              <DetailsModal collateral={data.collateral} entryPrice={data.entryPrice} isLongPosition={isLongPosition} leverage={data.leverage} liquidationPrice={data.liquidationPrice}
                markPrice={data.markPrice} netValue={data.netValue} open={open} position={data.position} positionType={data.positionType} setOpen={setOpen}
                size={data.size} token={data.token} />
            </>
          );
        })}
      </tbody>
