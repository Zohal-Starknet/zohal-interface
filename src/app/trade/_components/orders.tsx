      ...
      <table className="w-full">
        ...
        <tbody>
          {orders.map((order, index) => {
            const positionLeverageETH =
              Number(order.size_delta_usd) /
              10 ** 16 /
              (Number(order.initial_collateral_delta_amount) *
                Number(order.acceptable_price));
            const entry_price =
              Number(
                BigInt(order.size_delta_usd!) /
                  BigInt(order.initial_collateral_delta_amount!) /
                  BigInt(10 ** 16),
              ) / positionLeverageETH;
            ...
            return (
              <tr
                ...
                key={order.transaction_hash || index}
              >
                ...
                <td>
                  {order.initial_collateral_token == ETH_CONTRACT_ADDRESS
                    ? "ETH"
                    : "USDC"}
                </td>
